...
describe(`plugin-board`, () => {
  describe(`service`, () => {
    ...
    describe(`#ping()`, () => {
      it(`pings board service`, () => assert.isFulfilled(participants[0].spark.board.ping()));
    });
    ...
    describe(`#getChannels`, () => {
      ...
      it(`retrieves all boards for a specified conversation across multiple pages`, () => {
        const numChannelsToAdd = 22;
        const pageLimit = 20;
        let convo;
        // ensure we start fresh with 0 boards
        return participants[0].spark.conversation.create({
          displayName: `Test Board Listing`,
          participants
        })
          .then((c) => {
            convo = c;
          ...
          .then(() => {
            ...
            for (let i = 0; i < numChannelsToAdd - 1; i++) {
          ...
          .then((channelPage) => {
            assert.lengthOf(channelPage.items, 2);
            assert(!channelPage.hasNext());
          });
      });
    });
